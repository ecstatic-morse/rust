...
impl Qualifs<'a, 'mir, 'tcx> {
    ...
    fn in_return_place(&mut self, body: &Body<'tcx>) -> QualifSet {
        // Find the `Return` terminator, if one exists. If no `Return` terminator exists, the MIR
        // will always panic; mark it as unpromotable.
        ...
        let return_block = match return_block {
            None => return QualifSet::UNPROMOTABLE,
            Some(bb) => bb,
        };

        let return_loc = body.terminator_loc(return_block);
